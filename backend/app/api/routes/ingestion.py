...
from app.schemas.source import IndexRequest
...
def start_indexing(project_id: UUID, payload: IndexRequest, db: Session = Depends(get_db)) -> dict[str, str]:
    service = IngestionService(db)
    if not service.project_exists(project_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Project not found")
    task_id = service.enqueue_project_index(project_id, request=payload)
